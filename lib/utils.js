...
export async function runPythonScript(args) {
  ...
  try {
    const result = await execFileAsync(pythonBin, [PYTHON_SCRIPT, encoded], {
    ...
    const errObj = err || {};
    const stderrStr = typeof errObj.stderr === 'string'
      ? errObj.stderr
      : errObj.stderr
        ? String(errObj.stderr)
        : '';
    const msg = stderrStr.trim() || err.message || 'Python execution failed';
