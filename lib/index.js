...
const OUTPUT_LOG_FILE = path.join(__dirname, '..', 'config', 'output_log.json');
const MAX_OUTPUT_HISTORY = 50;

/**
 * Append a generated output entry to the output log.
 */
function recordOutput(entry) {
  try {
    let log = { version: 1, outputs: [] };
    if (existsSync(OUTPUT_LOG_FILE)) {
      log = JSON.parse(readFileSync(OUTPUT_LOG_FILE, 'utf8'));
    }
    log.outputs.unshift({
    ...
    log.outputs = log.outputs.slice(0, MAX_OUTPUT_HISTORY);
    writeFileSync(OUTPUT_LOG_FILE, JSON.stringify(log, null, 2), 'utf8');
  } catch (_) {}
}
...
export function apply(ctx) {
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    execute: async (args) => {
      try {
        ...
        if (result.success) {
          recordOutput({ tool: 'sprite_sheet', path: result.output_path || args.output_path, label: 'Sprite sheet' });
        }
        return result;
      } catch (err) {
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    execute: async (args) => {
      try {
        ...
        recordOutput({ tool: 'sprite_generate_image', path: outPath, label: 'Generated sprite sheet' });
        return {
  ...
  // Register cutout + validation tool (enhanced with solid/checkerboard/auto/mask_only)
  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_cutout',
    description: 'Apply background cutout with multiple modes (solid/checkerboard/auto/mask_only). Returns QC report with alpha transparency, bbox, connected components, checkerboard score, and white-fringe score.',
    parameters: {
      type: 'object',
      properties: {
        image_path: { type: 'string', description: 'Input image path' },
        output_path: { type: 'string', default: './output/cutout.png' },
        mode: { type: 'string', default: 'auto', enum: ['solid', 'checkerboard', 'auto', 'mask_only'], description: 'solid=single colour, checkerboard=alternating bg, auto=detect, mask_only=output mask only' },
        dist_threshold: { type: 'integer', default: 60, description: 'RGB Euclidean threshold (legacy, not used in Lab mode)' },
        lab_threshold: { type: 'number', default: 22.0, description: 'Lab colour distance threshold for background removal' },
        feather_radius: { type: 'integer', default: 1, description: 'Feather edge radius (0=hard edge)' },
        decontaminate_edges: { type: 'boolean', default: true, description: 'Reduce colour spill on semi-transparent edges' },
        remove_small_components: { type: 'boolean', default: false },
        min_component_area: { type: 'integer', default: 4 },
        save_mask_only: { type: 'boolean', default: false },
        overwrite: { type: 'boolean', default: false, description: 'Allow overwriting existing output file' },
      },
      required: ['image_path']
    },
    output: {
      ...
      render: (_a, v) => [{
        type: 'text',
        text: v.success
          ? `Cutout done — mode=${v.mode}  ${v.output_path}\n  transparent=${v.validation?.transparent_ratio_pct ?? '?'}%\n  corners=${v.validation?.corners_ok ?? '?'}  border=${v.validation?.border_ratio ?? '?'}\n  components=${v.validation?.connected_components ?? '?'}  checkerboard=${v.info?.checkerboard_score ?? '?'}`
          : `Error: ${v.error}`
      }]
    },
    execute: async (args) => {
      try {
        const { safeWrite } = await import('./safe_write.js');
        const outputPath = args.output_path || './output/cutout.png';
        const swResult = safeWrite(Buffer.alloc(0), outputPath, { overwrite: args.overwrite ?? false, dry_run: true });
        if (!swResult.success) {
        ...
        const result = await runPythonScript({
          ...
          lab_threshold: args.lab_threshold ?? 22.0,
          feather_radius: args.feather_radius ?? 1,
          decontaminate_edges: args.decontaminate_edges ?? true,
          remove_small_components: args.remove_small_components ?? false,
          min_component_area: args.min_component_area ?? 4,
          save_mask_only: args.save_mask_only ?? false,
        });
        return result;
      } catch (err) {
        return { success: false, error: err.message };
  ...
  // --- Sprite QC / Analysis Tool --------------------------------------------

  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_analyze',
    description: 'Run comprehensive QC analysis on a sprite image. Returns pass/fail, severity (P0/P1/P2), per-frame metrics (alpha ratio, bbox, edge contact, fringe score, checkerboard score, connected components), grid validation, and recommended_action.',
    parameters: {
      type: 'object',
      properties: {
        image_path: { type: 'string', description: 'Input image path' },
        grid_cols: { type: 'integer', default: 0, description: 'Grid columns (0=full image)' },
        grid_rows: { type: 'integer', default: 0, description: 'Grid rows (0=full image)' },
        cell_size: { type: 'integer', default: 32, description: 'Cell size in pixels' },
        regions: {
          type: 'array',
          description: 'Explicit regions [{name,x,y,w,h}] — overrides grid_cols/grid_rows',
          items: { type: 'object', properties: { name: { type: 'string' }, x: { type: 'integer' }, y: { type: 'integer' }, w: { type: 'integer' }, h: { type: 'integer' } } }
        },
      },
      required: ['image_path']
    },
    output: {
      ...
      render: (_a, v) => [{
        type: 'text',
        text: v.passed
          ? `QC PASS (${v.severity}) — ${v.recommended_action}`
          : `QC FAIL (${v.severity}) — ${v.failures?.join(' | ') || v.recommended_action}`
      }]
    },
    execute: async (args) => {
      try {
  ...
  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_edit',
    description: 'Send a local image to an AI provider for inpainting/editing. Supports transparent_background, preserve_pose, preserve_canvas, preserve_subject. Runs postprocess cutout and QC before writing output; P0 failures do not overwrite the final file.',
    parameters: {
      type: 'object',
      properties: {
        image_path: { type: 'string', description: 'Local image path to edit' },
        prompt: { type: 'string', description: 'Edit instruction' },
        output_path: { type: 'string', default: './output/edited.png' },
        provider: { type: 'string', default: 'gemini_flash' },
        width: { type: 'integer', default: 1024 },
        height: { type: 'integer', default: 1024 },
        preserve_pose: { type: 'boolean', default: true },
        preserve_canvas: { type: 'boolean', default: true },
        preserve_subject: { type: 'boolean', default: true },
        transparent_background: { type: 'boolean', default: false },
        negative_prompt: { type: 'string', default: '' },
        postprocess: { type: 'boolean', default: true },
    ...
    output: {
      ...
      render: (_a, v) => [{
        type: 'text',
        text: v.success
          ? `Edit done: ${v.output_path}  qc=${v.qc?.severity ?? '?'}`
          : `Error: ${v.error}`
      }]
    },
    execute: async (args) => {
      try {
        const { editSprite } = await import('./sprite_edit.js');
        return await editSprite(args, ctx);
      } catch (err) {
        return { success: false, error: err.message };
      }
    }
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    execute: async (args) => {
      const result = await generateAnimationSequence(args, ctx);
      if (result.success && result.output_path) {
        recordOutput({ tool: 'sprite_animation_sequence', path: result.output_path, label: `Animation: ${args.sequence}` });
      }
      return result;
    }
  }));
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    execute: async (args) => {
      const result = await generateCharacterConsistent(args, ctx);
      if (result.success && result.output_path) {
        recordOutput({ tool: 'sprite_character_generate', path: result.output_path, label: `Character: ${args.character_id || 'new'}` });
      }
      return result;
    }
  }));
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    execute: async (args) => {
      const result = await generateCharacterSheet(args, ctx);
      if (result.success && result.output_path) {
        recordOutput({ tool: 'sprite_character_sheet', path: result.output_path, label: `Character sheet: ${args.character_id || 'new'}` });
      }
      return result;
    }
  }));
  ...
  // --- Terrain Tile Bundle v2 (QC + edge_policy) ----------------------------

  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_tileset_batch_qc',
    description: 'Enhanced terrain tile bundle generator with edge policy (isolated/seamless), per-variant contrast QC, pairwise PSNR variant-diff QC, and material hints. Outputs tileset-bundle.json v2 manifest.',
    parameters: {
      type: 'object',
      properties: {
        image_path: { type: 'string', description: 'Input terrain atlas PNG' },
        output_dir: { type: 'string', default: './output/tileset_qc/' },
        terrain_rows: { type: 'array', items: { type: 'object' }, description: '[{name, row}] e.g. [{name:"grass",row:0},{name:"dirt",row:1}]' },
        cols: { type: 'integer', default: 8 },
        rows: { type: 'integer', default: 4 },
        tile_size: { type: 'integer', default: 32 },
        min_contrast: { type: 'number', default: 0.05 },
        edge_policy: { type: 'string', default: 'isolated', enum: ['isolated', 'seamless'], description: 'isolated=transparent gaps, seamless=edge-blended' },
        max_variants: { type: 'integer', default: 4 },
      },
      required: ['image_path']
    },
    output: {
      schema: { type: 'object' },
      render: (_a, v) => v.success
        ? [{ type: 'text', text: `Tileset QC: ${v.terrains} terrains, ${v.total_tiles} tiles, edge=${v.edge_policy}, low_contrast=${v.qc_summary?.low_contrast_total ?? 0}` }]
        : [{ type: 'text', text: `Error: ${v.error}` }]
    },
    execute: async (args) => {
      try {
        return await runPythonScript({
          command: 'tileset_batch_v2',
          image_path: args.image_path,
          output_dir: args.output_dir || './output/tileset_qc/',
          terrain_rows: args.terrain_rows || [],
          cols: args.cols ?? 8,
          rows: args.rows ?? 4,
          tile_size: args.tile_size ?? 32,
          min_contrast: args.min_contrast ?? 0.05,
          edge_policy: args.edge_policy || 'isolated',
          max_variants: args.max_variants ?? 4,
        });
      } catch (err) {
        return { success: false, error: err.message };
      }
  ...
  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_platform_strip',
    description: 'Generate reusable side-scrolling platform strips (left cap / middle repeat / right cap) from a sprite sheet. Returns per-piece PNGs + collision metadata JSON.',
    parameters: {
      type: 'object',
      properties: {
        image_path: { type: 'string', description: 'Input platform strip sheet PNG' },
        platform_type: { type: 'string', default: 'grass', enum: ['grass', 'stone', 'wood', 'ice', 'danger'], description: 'Base terrain type' },
        style: { type: 'string', description: 'Style variant (auto-inferred if omitted)' },
        output_dir: { type: 'string', default: './output/platforms/' },
        cell_width: { type: 'integer', default: 256 },
        cell_height: { type: 'integer', default: 64 },
        chroma_key: { type: 'array', default: [255, 0, 255] },
        tolerance: { type: 'integer', default: 40 },
      },
      required: ['image_path']
    ...
    output: {
      schema: { type: 'object' },
      render: (_a, v) => v.success
        ? [{ type: 'text', text: `Platform strip "${v.platform_type}": ${v.pieces_extracted} pieces → ${v.output_dir}` }]
        : [{ type: 'text', text: `Error: ${v.error}` }]
    },
    execute: async (args) => {
      try {
        return await runPythonScript({
          command: 'platform_strip',
          image_path: args.image_path,
          platform_type: args.platform_type || 'grass',
          style: args.style,
          output_dir: args.output_dir || './output/platforms/',
          cell_width: args.cell_width ?? 256,
          cell_height: args.cell_height ?? 64,
          chroma_key: args.chroma_key ?? [255, 0, 255],
          tolerance: args.tolerance ?? 40,
        });
      } catch (err) {
  ...
  // --- Preview / Output Log Tools -------------------------------------------

  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_preview_list',
    description: 'List recently generated sprite outputs for preview. Returns paths and metadata for the last N items.',
    parameters: {
      type: 'object',
      properties: {
    ...
    output: {
      ...
      render: (_a, v) => [{
        type: 'text',
        text: v.success
          ? `Recent outputs (${v.count}): ${v.outputs.map(o => `${o.label} → ${o.path}`).join(', ')}`
          : `Error: ${v.error}`
      }]
    },
    execute: async (args) => {
      try {
        ...
        return { success: true, count: log.outputs.length, outputs: log.outputs.slice(0, limit) };
      } catch (err) {
        return { success: false, error: err.message };
      }
    }
  ...
  harness.registerTool(ctx, harness.defineTool({
    name: 'sprite_preview_image',
    description: 'Read a local image file and return it as a base64 data URL for in-client preview rendering.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Absolute or relative path to image file' },
        max_width: { type: 'integer', default: 512, description: 'Max width for thumbnail' },
      },
      required: ['path'],
    },
    output: {
      schema: { type: 'object' },
      render: (_a, v) => v.success
        ? [{ type: 'image', source: { type: 'base64', media_type: 'image/png', data: v.data_url } }]
        : [{ type: 'text', text: `Error: ${v.error}` }]
    },
    execute: async (args) => {
      try {
        ...
        return { success: true, data_url: `data:${mimeType};base64,${buffer.toString('base64')}`, size: buffer.length };
      } catch (err) {
        return { success: false, error: err.message };
      }
    }
